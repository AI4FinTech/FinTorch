...
def test_attention_mask_shape():
    ...
    assert mask.shape == (past_length + future_length, past_length + future_length), "Mask shape is incorrect"
...
def test_attention_mask_upper_triangle():
    ...
    for i in range(total_length):
        for j in range(i + 1, total_length):
            assert mask[i, j] is True, f"Mask at position ({i}, {j}) should be True"
...
def test_attention_mask_lower_triangle():
    ...
    for i in range(total_length):
        for j in range(i + 1):
            assert mask[i, j] is False, f"Mask at position ({i}, {j}) should be False"
