...
class StockTicker(InMemoryDataset):
    ...
    def download(self):
        ...
        raw_data = yf.download(self.tickers,
        ...
        hist = df.melt(ignore_index=False,
                       var_name="Ticker",
    ...
    def process(self) -> None:
        ...
        wide_df = concatenated_df.pivot(index="ds",
                                        columns="unique_id",
                                        values="y")
    ...
    def spatial_graph_construction(self, df):
        ...
        edge_index = torch.tensor(result_df.select(["src",
                                                    "dst"]).to_numpy().T,
                                  dtype=torch.long)
        edge_attr = torch.tensor(result_df["weight"].to_numpy(),
                                 dtype=torch.float32)  # Adjust dtype if needed
